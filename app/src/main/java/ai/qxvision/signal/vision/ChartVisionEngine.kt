...
class ChartVisionEngine {
    ...
    fun inspect(bitmap: Bitmap, duplicate: Boolean=false): DetectedChart {
        ...
        val marginX=max(2,bitmap.width/16); val marginY=max(2,bitmap.height/16); val xStart=max(0,bitmap.width-marginX); val yStart=max(0,bitmap.height-marginY); val rightLine=(xStart until bitmap.width).maxOfOrNull{x:Int->(0 until bitmap.height).count{y->dist(bitmap.getPixel(x,y),bg)>.08f}.toFloat()/max(1,bitmap.height)}?:0f; val bottomLine=(yStart until bitmap.height).maxOfOrNull{y:Int->(0 until bitmap.width).count{x->dist(bitmap.getPixel(x,y),bg)>.08f}.toFloat()/max(1,bitmap.width)}?:0f; val axis=max(rightLine,bottomLine).coerceIn(0f,1f); val indicator=((saturated-matched).toFloat()/max(1,count)*12f).coerceIn(0f,1f); val occlusion=(indicator*.65f+(1f-visible)*.35f).coerceIn(0f,1f)
